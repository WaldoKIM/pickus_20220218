...
class My_Mail 
{
    ...
    $this->to = mb_ereg_replace(";", ",", $this->to);
	$this->cc = mb_ereg_replace(";", ",", $this->cc);
	$this->bcc = mb_ereg_replace(";", ",", $this->bcc);
    ...
    function email_check($emails) 
    {
		...
		//   if(eregi("<(.+)>", $emails, $match)) $email = $match[1];
		   if(preg_match("<(.+)>", $emails, $match)) $email = $match[1];
...
class My_Mime_Mail extends My_Mail 
{
 ...
 $this->subject = iconv("UTF-8","EUC-KR",stripslashes(trim($this->subject))); 
