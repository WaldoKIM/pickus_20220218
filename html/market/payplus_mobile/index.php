...
if(!$_GET[TRADE_CODE]) { $tools->alertJavaGo('잘못된 주문입니다\n\n처음부터 다시 주문하세요', '../mobile_skin/cart.php');}
$order_stat=$db->object("cs_trade_tmp", "where item=1 and code is not null and code='$_GET[TRADE_CODE]' order by idx asc");
$order_stat_data= $tools->decode( $order_stat->data );
$trade_stat=$db->object("cs_trade_tmp", "where item=2 and code is not null and code='$_GET[TRADE_CODE]' order by idx asc");
$trade_stat_data= $tools->decode( $trade_stat->data );

$trade_result=$db->select("cs_trade_tmp", "where item=0 and code is not null and code='$_GET[TRADE_CODE]' order by idx asc");
...
while($trade_row=@mysqli_fetch_object($trade_result)) {
	...
	$total_goods_box+=$trade_data[box];
	if($form_cnt==1) $goodsnames = $db->addSlash(urldecode($trade_data[goods_name]));
	
}
if($form_cnt > 1){
	$form_cnt = $form_cnt - 1;
	$goodsnames = $goodsnames." 외".$form_cnt."건";
}
...
<?
  ...
  $url = "https://".$_SERVER["SERVER_NAME"]."/market/payplus_mobile/pp_ax_hub.php";
?>
...
<form name="order_info" method="post">
  ...
  <div class="sample">
    ...
    <table class="tbl" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td>
			...
			<?}else if($trade_stat_data[trade_method]==2){?>
                <option value="acnt" selected>계좌이체</option>
			<?}else if($trade_stat_data[trade_method]==4){?>
                <option value="vcnt" selected>가상계좌</option>
			<?}else if($trade_stat_data[trade_method]==3){?>
                <option value="mobx" selected>휴대폰</option>
			<?}?>
            </select>
        </td>
      </tr>
      <tr>
        <th>주문 번호</th>
        <td><input type="text" name="ordr_idxx" class="w200" value="<?=$_GET[TRADE_CODE];?>" readonly></td>
      </tr>
      <tr>
        <th>상품명</th>
        <td><input type="text" name="good_name" class="w100" value="<?=$goodsnames;?>" readonly></td>
      </tr>
      <tr>
        <th>결제 금액</th>
        <td><input type="text" name="good_mny" class="w100" value="<?=$trade_stat_data[trade_price];?>" readonly></td>
      </tr>
      <tr>
        <th>주문자명</th>
        <td><input type="text" name="buyr_name" class="w100" value="<?=$order_stat_data[order_name];?>" readonly></td>
      </tr>
      <tr>
        <th>E-mail</th>
        <td><input type="text" name="buyr_mail" class="w200" value="<?=$order_stat_data[order_email];?>" readonly></td>
      </tr>
      <tr>
        <th>전화번호</th>
        <td><input type="text" name="buyr_tel1" class="w100" value="<?=$order_stat_data[deliv_tel1];?><?=$order_stat_data[deliv_tel2];?><?=$order_stat_data[deliv_tel3];?>" readonly></td>
      </tr>
      <tr>
        <th>휴대폰번호</th>
        <td><input type="text" name="buyr_tel2" class="w100" value="<?=$order_stat_data[deliv_phone1];?><?=$order_stat_data[deliv_phone2];?><?=$order_stat_data[deliv_phone3];?>" readonly></td>
      </tr>
    </table>
    ...
    <table class="tbl" cellpadding="0" cellspacing="0" >
      <tr>
        <th>수취인이름</th>
        <td><input type="text" name="rcvr_name" class="w100" value="<?=$order_stat_data[deliv_name];?>" readonly></td>
      </tr>
      <tr>
        <th>수취인 연락처</th>
        <td><input type="text" name="rcvr_tel1" class="w100" value="<?=$order_stat_data[deliv_tel1];?><?=$order_stat_data[deliv_tel2];?><?=$order_stat_data[deliv_tel3];?>" readonly></td>
      </tr>
      <tr>
        <th>수취인 휴대폰번호</th>
        <td><input type="text" name="rcvr_tel2" class="w100" value="<?=$order_stat_data[deliv_phone1];?><?=$order_stat_data[deliv_phone2];?><?=$order_stat_data[deliv_phone3];?>" readonly></td>
      </tr>
      <tr>
        <th>수취인 E-mail</th>
        <td><input type="text" name="rcvr_mail" class="w200" value="<?=$order_stat_data[deliv_email];?>" readonly></td>
      </tr>
      <tr>
        <th>수취인 우편번호</th>
        <td><input type="text" name="rcvr_zipx" class="w100" value="<?=$order_stat_data[deliv_zip];?>" readonly></td>
      </tr>
      <tr>
        <th>수취인 주소</th>
        <td><input type="text" name="rcvr_add1" class="w300" value="<?=$order_stat_data[deliv_add1];?>" readonly></td>
      </tr>
      <tr>
        <th>수취인 상세주소</th>
        <td><input type="text" name="rcvr_add2" class="w300" value="<?=$order_stat_data[deliv_add2];?>" readonly></td>
      </tr>
    </table>
	<?}?>
    ...
    <div class="btnset" id="display_pay_button" style="display:block">
      ...
      <a href="../mobile_skin/cart.php" class="home">처음으로</a>
    </div>
  </div>
  <!--footer-->
  <div class="footer">
...
<input type="hidden" name="PayUrl"   id="PayUrl"   value=""/> <!--( 소스에 없는 경우만 추가)-->
</form>
</div>
