		...
		<td valign="top">
			<table width="100%" style='border-collapse: collapse'>
				...
				<tr>
					...
					<td height="45"  id="sel_01" align="left">
						<select name='category' class='formSelect'>
				...
				<tr <?if($_SESSION[USERID]){?>style="display:none;"<?}?>>
					...
					<td height="45" style="padding-left:3px" align="left">
						<input type="text" name="name" class="formText" maxlength="15" value="<?=$bbs_stat->name;?>">
					</td>
				</tr>
				<tr <?if($_SESSION[USERID]){?>style="display:none;"<?}?>>
					<td width="7" colspan="2" height="1" bgColor='#dddddd'></td>
				...
				<tr <?if($_SESSION[USERID]){?>style="display:none;"<?}?>>
					...
					<td height="45" style="padding-left:3px" align="left">
						<input type="password" name="pwd" class="formText" maxlength="15" value="<?=$_SESSION[PASSWD];?>">
					</td>
				</tr>
				<tr <?if($_SESSION[USERID]){?>style="display:none;"<?}?>>
					<td width="7" colspan="2" height="1" bgColor='#dddddd'></td>
				...
				<?for($i=1;$i<=5;$i++){?>
				<tr name="add_view<?=$i?>" id="add_view<?=$i?>" style="display:<?if($bbs_stat->{"add_file".$i} != "none"){?><?}else{?>none<?}?>"> 
					<td height="65" align="center" bgcolor="#F2F2F2" class='oolimmobilemenuM'>추가파일<?=$i?></td>
					<td style="padding:5px" align="left"><input type="file" class='file_text' maxlength="15" name="add_file<?=$i?>"><?if($bbs_stat->{"add_file".$i} != "none"){?>&nbsp;<a href="../data/bbsData/<?=$bbs_stat->{"add_file".$i};?>" rel="lightbox" class="smallBtn07" style="width:60px">미리보기</a>&nbsp;<input type="checkbox" value="1" name="del_add_file<?=$i?>">삭제시 선택하여주세요[삭제우선]!<?}?>
					</td>
				</tr>
				...
				<tr>
					...
					<td height="45" style="padding:5px" align="left">
						&nbsp;<a href="javascript:add_file();" class="smallBtn07" style="width:60px">파일추가</a>파일추가는 5개까지 가능합니다.</a>
					</td>
				</tr>
				<tr>
