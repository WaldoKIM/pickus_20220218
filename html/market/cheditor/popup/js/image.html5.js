...
function doUpload() {
    ...
    reader.addEventListener("error", function(ev) {
        ...
        switch (ev.target.error.code) {
            ...
            case ev.target.error.ABORT_ERR:
                ...
                break;
            case ev.target.error.SECURITY_ERR:
                status = "파일이 잠겨 있습니다.";
                break;
            case ev.target.error.ENCODING_ERR:
                status = "data:// URL의 파일 인코딩 길이가 너무 깁니다.";
                break;
            default:
                status = "파일 읽기 오류: " + ev.target.error.code;
        }
        alert("'"+ev.target.filename+"' "+ status);
    }, false);
    ...
    reader.addEventListener("load", function(readerEvent) {
        ...
        origImage.addEventListener("load", function() {
            ...
            for (i=0; i<el.length; i++) {
                imgBox = el[i];
                if (imgBox.className !== 'imageBox_theImage') { continue; }
                if (!imgBox.firstChild || imgBox.firstChild.tagName.toLowerCase() !== 'img') {
                    showProgress = imgBox;
                    break;
                }
            ...
            xhr.addEventListener("error", function(evt) { alert("파일 전송 중 오류: " + evt.target.error.code); }, false);
        ...
        var objectUrl = URL.createObjectURL(readerEvent.target.file);
