...
if(!$_SESSION[USERID]){
	...
	if($_GET[cart_method]==1) {
		...
		$cart_goods_cnt = $db->object("cs_cart", "where goods_idx=$idx and code='$_SESSION[CART]' and opt_data='$Optdata'");
		if(($goods_stat->unlimit==0) && ($goods_stat->number < ($cart_goods_cnt->goods_cnt + $_POST[buy_goods_cnt]))) {
			...
			if(empty($cart_goods_cnt->goods_cnt)) {
				// 따음표 문제 해결
				$db->insert("cs_cart", "userid='$_SESSION[USERID]', code='$_SESSION[CART]', part_idx=$part_idx, goods_idx=$idx, goods_code=$goods_stat->code, goods_name='$goods_stat_name', goods_price=$shopPrice, goods_point=$goods_stat->point, goods_cnt=$_POST[buy_goods_cnt], opt_data='$Optdata', box=$goods_stat->box, register=now()");
			} else {
...
<body text="black" link="blue" vlink="purple" alink="red" leftmargin="0" marginwidth="0" topmargin="10" marginheight="0" style="overflow-y:hidden;overflow-x:hidden">
<table width="100%">
    <tr>
...
<table width="100%">
    <tr>
        <td height="100" align="center"><!--<a href="javascript:parent.close_cenlayer()"><img src="skinimage/main_popupicon_close.gif" border="0"></a> --><a href="cart.php" target="_parent" style='width:100px;' class='btn-type6_view'>장바구니로 이동</a></td>
	</tr>
</table>