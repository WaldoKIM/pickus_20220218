...
<!--PC 화면용 리스트-->
	...
	<table width="100%" style="display:none">
		<tr>
			<td width="40%" align="center" class='main_noticeL' style="CURSOR:pointer;"  title='공지사항'>공지사항</td>
			<td width="40%" align="center" onclick="hiddenView('noticeqna','1')" class='main_noticeQ' style="CURSOR:pointer;" title='질답게시판'>질답게시판</td>
...
<div class="main_notice_contsL">
	<div id='main_notice_box'>
		<div id='main_notice_box_left1'></div>
		<h3>공지사항<a href="bbs_list.php?code=<?=$code?>" title="전체보기">더보기 +</a></h3>
		<div id='main_notice_box_right'>
			<table width="100%">
				...
				while( $notice_row = @mysqli_fetch_object($notice_result)) {
					...
					if( $new_check ) {	$new_img			=		$page->bbsNewImg( $notice_row->reg_date, $bbs_admin_stat->new_mark, "&nbsp;<img src='./images/icon_new1.gif' style='vertical-align:middle;' border='0'>" ); }
				...
				?>
				<tr>
					<td class='notice_subI'><a href="bbs_list.php?boardT=v&board_data=<?=$bbs_data;?>&search_items=<?=$SEARCH_DATA?>"><?=$db->stripSlash($subject);?></a><?=$new_img?>
					</td>
				</tr>
				<?}?>
			</table>
