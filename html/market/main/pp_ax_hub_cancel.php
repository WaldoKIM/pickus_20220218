...
<?
    ...
    /* =   02. 지불 요청 정보 설정                                                  = */
    /* = -------------------------------------------------------------------------- = */
    include "../payplus_mobile/cfg/site_conf_inc.php";       // 환경설정 파일 include
    require "./pp_ax_hub_lib.php";              // library [수정불가]    
    /* = -------------------------------------------------------------------------- = */
    $req_tx         = $_POST[ "req_tx"         ];	// 필수!(mod) 요청 종류		
	$tran_cd        = $_POST[ "tran_cd"        ];   // 처리 종류	
    $cust_ip        = getenv( "REMOTE_ADDR"    );   // 요청 IP
    /* = -------------------------------------------------------------------------- = */
    $res_cd         = "";                                                     // 응답코드
    $res_msg        = "";                                                     // 응답 메세지
    $tno            = $_POST[ "tno"       	  ];                              // 필수!(거래고유번호) KCP 거래 고유 번호
    ...
    /* = -------------------------------------------------------------------------- = */
    $mod_type       = $_POST[ "mod_type"      ];                              // 필수!(STSC) 변경TYPE(승인취소시 필요)
    $mod_desc       = $_POST[ "mod_desc"      ];                              // 필수!(EX고객변심) 변경사유
    ...
    if ( $tran_cd != "" )
    {
        ...
        $c_PayPlus->mf_do_tx( "", $g_conf_home_dir, $g_conf_site_cd, $g_conf_site_key, $tran_cd, "",
                             ...
                             "", "3" , 1, 0, $g_conf_key_dir, $g_conf_log_dir); // 응답 전문 처리
    }
    else
    {
	 ...
	 if ( $mod_type == "STPC") //부분 취소 정상결제인 경우 
  	{
		...
		$point_mny 		    = $c_PayPlus->mf_get_res_data( "point_mny" ); 
	...
	else if ( $res_cd == "0000" && $mod_type == "STPC")
	{
        ...
        echo "결과메세지 :$res_msg    <p>";
        echo "<script> alert(\"결제취소가 완료되었습니다.\");</script>";
	}
    else
    {
...
<body onload="goResult()">
    <form name="pay_info" method="post" action="./cancel_refund_ok.php">
