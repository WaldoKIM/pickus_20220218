			...
			<div class="row">
				<div class="to_animate">
					...
					<div class="main" style="text-align:left;">
						...
						<span class="mypage_font product-viewboxM"><i style="display:none;" class='fa-certificate_color fa-chevron-circle-right'></i>회원정보</span>
						<!---------회원정보출력------------->
						<div class='my_orderview_pc'>
							<table width="100%">
								<tr>
									<td align="center">
										<table width="100%" class="mypage_border jointable_all">
											<tr>
												<td align="center">
													<? $mem_row = $db->object("cs_member", "where userid='$_SESSION[USERID]' and passwd='$_SESSION[PASSWD]'");?>
													<table width="100%">
														<tr style="display:none;" class='jointable_td'>
															<?/*
															...
															*/?>
															<td style="display:none;" width="80" bgcolor='efefef' style='text-align:center;' class='oolimmobilemenuM'>휴대폰</td>
															<td colspan="3" style='padding-left:10px;'><?=$mem_row->phone1;?> <?=$mem_row->phone2;?> <?=$mem_row->phone3;?></td>
														</tr>
														<tr class='mypage_design jointable_td'>
															<td style="display:none;" width="80" bgcolor='efefef' style='text-align:center;' class='oolimmobilemenuM'>휴대폰</td>
															<td colspan="3" style='width:30%; padding-left:10px;'><div class="mypage_design_font">휴대폰</div><?=$mem_row->phone1;?> <?=$mem_row->phone2;?> <?=$mem_row->phone3;?></td>
															<td style="display:none;" width="80" style='text-align:center;' bgcolor='efefef' class='oolimmobilemenuM'>이메일</td>
															<td style='width:30%; padding-left:10px;'><div class="mypage_design_font">이메일</div><?=$mem_row->email;?></td>
															<td style="display:none;" 
															width="80" style='text-align:center;' bgcolor='efefef' class='oolimmobilemenuM'>적립금</td>
															<td style='width:30%; padding-left:10px;'><div class="mypage_design_font">적립금</div><? $total_point = $db->sum("cs_point", "where userid='$_SESSION[USERID]'", "point"); echo(number_format($total_point));?>원
															</td>
						...
						<div class='my_orderview_mobile'>
							<table width="100%">
								<tr>
									<td>
										<table width="100%" class="mypage_design jointable_all">
											<? $mem_row = $db->object("cs_member", "where userid='$_SESSION[USERID]' and passwd='$_SESSION[PASSWD]'");?>
											<?/*
											<tr>
											...
											*/?>
											<tr>
												<td class="mypage_border order-viewboxIcon jointable_td_border4"><div class="mypage_design_font">휴대폰</div> <?=$mem_row->phone1;?><?=$mem_row->phone2;?><?=$mem_row->phone3;?></td>
											</tr>
											<tr>
												<td class="mypage_border order-viewboxIcon jointable_td_border4"><div class="mypage_design_font">이메일</div><?=$mem_row->email;?></td>
											</tr>
											<tr>
												<td class="mypage_border order-viewboxIcon jointable_td_border4"'><div class="mypage_design_font">적립금</div><? $total_point = $db->sum("cs_point", "where userid='$_SESSION[USERID]'", "point"); echo(number_format($total_point));?>원</td>
											</tr>
										</table>
									</td>
								</tr>
					...
					<tr>
					  <td valign="top">
						...
						<span class="mypage_font product-viewboxM"><i style="display:none;" class='fa-certificate_color fa-chevron-circle-right'></i>최근주문내역</span>
						<!-------주문내역조회--------->
						<table width="100%">
							<tr>
								<td>
									<table width="100%" class="mypage_border jointable_all">
										...
										?>
										<tr class="mypage_design" id='calendar_list_tableTD_on'>
											...
											<td  class='mypage_border calendar_list_tableTD_bg noneoolim_del' style='text-align:center;'>
												<div class="mypage_design_font">주문번호</div><a href="javascript:tradeView('<?=$trade_data;?>');"><?=$my_trade_row->trade_code;?></a>
											</td>
											...
											<td  style='text-align:center;padding:3px;' class='mypage_border calendar_list_tableTD_bg'>
												<?
												$itemN="";
												...
												?>
												<div class="mypage_design_font">상품명</div><a href="javascript:tradeView('<?=$trade_data;?>');"><?=$tools->strCut($db->stripSlash($trade_goods_row->goods_name), 100);?></a>&nbsp;<a href="product_view.php?part_idx=<?=$trade_goods_row->part_idx;?>&goods_data=<?=$goods_data;?>" class='Btn_orderlist5'>상품평작성</a>
												<?}?>
											</td>
											<td style='text-align:center;padding:3px;' class='mypage_border calendar_list_tableTD_bg price noneoolim_del2'><div class="mypage_design_font">주문금액</div><?=number_format($my_trade_row->trade_price);?>원</td>
											<td style='text-align:center;padding:3px;' class='mypage_border calendar_list_tableTD_bgright'>
												...
												<? if($my_trade_row->trade_stat==1) {?><span class='Btn_orderlist1'>결제대기</span><?} else if($my_trade_row->trade_stat==2){?><span class='Btn_orderlist2'>결제확인됨</span><?} else if($my_trade_row->trade_stat==3){?><span class='Btn_orderlist3'>상품배송중</span><?} else if($my_trade_row->trade_stat==4){?><span class='Btn_orderlist3'>판매완료됨</span><?}?>
											</td>
										</tr>
										<?
										}
					...
					<tr>
					  <td valign="top">
						<div class='spaceline01'></div>
						<span style="display:none;" class="mypage_font product-viewboxM"><i style="display:none;" class='fa-certificate_color fa-chevron-circle-right'></i>내가찜한상품</span>
						<!-------내가찜한상품--------->
