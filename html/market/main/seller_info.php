...
<?
	...
	//카테고리에 대한 상품 총수량
	$total_goods=$db->cnt( "cs_goods", "where seller = '$_POST[search]' and display=1 $outPart " );
?>
...
<div class="site-wrapper" id="box_wrapper">
	...
	<!--페이지 위치-->
	<div style="display:none;" class="my_location">
		...
		<div class="container-fluid">
			<div class="row">
				<div class="to_animate">
					<!--********************콘텐츠 시작********************-->
							<div style="margin-top:10px; font-size: 16px; display: flex; justify-content: space-evenly; align-items: center;" class="search_result_area">
								<?
									$seller_stat = $db->object("cs_member","where userid='$_POST[search]'");
									$seller_stat2 = $db->object("g5_member","where mb_id='$_POST[search]'");
									
								?>
								<?php echo "<div class='img'> <img style='height: 85px;width: 85px;border-radius: 50%;margin-bottom: 5px;border: 1px solid #ededed;height:50px;' src = '/data/estimate/" . $seller_stat2->mb_photo_site . "'></div>"; ?>
								<?=mb_substr($seller_stat->name, 0, 1, 'utf-8');?> 전문가님
								<span style="font-size: 16px;" class="s_name">상품 : <span class="num"><?=$total_goods;?></span>개
							</div>
							<!--출력형태 버튼 -->
							<div class="filter_btn_area">
								<!--*****순서정렬 버튼 PC용*****-->
								<div class="cbp-vm-options_pc">
									<a href="?part_idx=<?=$part_idx;?>&search_item=1&search=<?=$_POST[search];?>" class="<?if($search_item==1){?> select<?}?>"><span></span>이름순</a>
									<a href="?part_idx=<?=$part_idx;?>&search_item=2&search=<?=$_POST[search];?>" class="<?if($search_item==2){?> select<?}?>"><span></span>가격낮은순</a>
									<a href="?part_idx=<?=$part_idx;?>&search_item=3&search=<?=$_POST[search];?>" class="<?if($search_item==3){?> select<?}?>"><span></span>가격높은순</a>
									<a href="?part_idx=<?=$part_idx;?>&search_item=4&search=<?=$_POST[search];?>" class="<?if($search_item==4){?> select<?}?>"><span></span>등록순</a>
									<a href="?part_idx=<?=$part_idx;?>&search_item=5&search=<?=$_POST[search];?>" class="<?if($search_item==5){?> select<?}?>"><span></span>인기순</a>
								</div>
								<!--*****순서정렬 버튼 모바일용*****-->
							...
							$totalList	= $db->cnt( $table, "where (seller = '$_POST[search]') and display=1 $sort" );
							$result		= $db->select( $table, "where (seller = '$_POST[search]')  and display=1 $sort LIMIT $startPage, $listScale" );
							...
							<div class="gallery-item col-lg-3" style='margin:0 auto;display:inline-block;vertical-align: top;'>
								<div class='gallery-image gallery_box_img_list'>
									<div class='gallery-image_front'>
										...
										*/?>
										<!--제품이미지-->
										<a href="product_view.php?part_idx=<?=$row->part_idx;?>&goods_data=<?=$goods_data;?>"><?if($row->resize==1){?><img style="width: 100%;max-width: 230px;max-height: 230px;border: 1px solid #ccc; box-sizing: content-box;" src="../data/goodsImages/<?=$row->images1?>" border="0"  alt="<?=$row->name?>" class='gallery_box_img'><?}else{?><img src="../data/goodsImages/<?=$row->images1?>" border="0"  alt="<?=$row->name?>" class='gallery_box_img' style='margin-bottom:10px;'><?}?></a>
									</div>	<!--gallery-image-->
									<!--제품명-->
									<span style="width:100%;" class='oolimmobilemenuL' style='display:inline-block;padding:0 0 0;'>
									<a href='product_view.php?part_idx=<?=$row->part_idx;?>&goods_data=<?=$goods_data;?>'>
										<p style="max-width:230px; margin:auto;" class="product_list_name"><?=$tools->strCut($row->name,45);?></p>
										<p style="max-width:230px; margin:auto;" class="product_list_mdname"><?=$tools->strCut($row->content,20);?></p>
									</a>
									<!--베스트,신상,히트 등 아이콘 (관리자등록)-->
									<?for($i=1;$i<count($arrIcon);$i++){
									if($arrIcon[$i] > 0){
									?><span class='gallery_content_icon'><img src="../data/designImages/<?=$arrPicon[$arrIcon[$i]]?>"></span><?}}?>
									</span>
