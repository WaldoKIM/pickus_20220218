...
if($_GET[cartidx] )					{ $cartidx = $_GET[cartidx]; }					else { $cartidx = $_POST[cartidx]; }
...
$tools->metaGo('login_check.php?login_go='.basename($_SERVER['PHP_SELF']).'?'.$_SERVER['QUERY_STRING']);
}  else {
				...
				<div class="to_animate">
		            ...
		            <div class="order_list_area">
						...
						while($cart_row=@mysqli_fetch_object($cart_result)) {
							...
							$trade_goods_data = $tools->encode("trade_code=".$TRADE_CODE."&part_idx=".$cart_row->part_idx."&goods_idx=".$cart_row->goods_idx."&goods_code=".$cart_row->goods_code."&goods_name=".$encode_goods_name."&goods_price=".$cart_row->goods_price."&goods_cnt=".$cart_row->goods_cnt."&goods_point=".$cart_row->goods_point."&opt_data=".$cart_row->opt_data."&box=".$cart_row->box);
							...
							$goods_info = $db->object("cs_goods","where code='$cart_row->goods_code' ");
						?>
						<ul class="data_cell">
							<li class="prd_photo"><a href="product_view.php?goods_data=<?=$goods_data;?>"><img src="../data/goodsImages/<?=$goods_info->images1?>" border="0"></a></li>
							...
							<li class="point"><span><?=number_format($cart_row->goods_price*$cart_row->goods_point*0.01);?></span>point</li>
						...
						while($cart_row=@mysqli_fetch_object($cart_result)) {
							...
							// trade tmp 에 저장한다.
							$encode_goods_name = urlencode($goods_name);
							$trade_goods_data = $tools->encode("trade_code=".$TRADE_CODE."&part_idx=".$cart_row->part_idx."&goods_idx=".$cart_row->goods_idx."&goods_code=".$cart_row->goods_code."&goods_name=".$encode_goods_name."&goods_price=".$cart_row->goods_price."&goods_cnt=".$cart_row->goods_cnt."&goods_point=".$cart_row->goods_point."&opt_data=".$cart_row->opt_data."&box=".$cart_row->box."&seller=".$cart_row->seller);
							$db->insert("cs_trade_tmp", "code='$TRADE_CODE', item=0, data='$trade_goods_data', register=now()");
							$ThumbEncode = $tools->encode("idx=".$cart_row->goods_idx."&table=cs_goods&img=images1&dire=goodsImages&w=70&h=70");
							$goods_info = $db->object("cs_goods","where code='$cart_row->goods_code' ");
						?>
						<ul class="data_cell">
							<li class="prd_photo"><a href="product_view.php?goods_data=<?=$trade_goods_data;?>"><img src="../data/goodsImages/<?=$goods_info->images1?>" border="0"></a></li>
							<li class="prd_name_option">
								<div><?=$goods_name;?><div>
								<?
								$optArr = explode("/^CUT/^", $cart_row->opt_data);
						...
						<?} else if($_GET[trade_method]==3) { //관심상품
							if(!$_SESSION[USERID] || !$_SESSION[PASSWD]) { $toods->msgClose('잘못된 접근입니다');}
							...
							while($withlist_row=@mysqli_fetch_object($wishlist_result)) {
								...
								// trade tmp 에 저장한다.
								$encode_goods_name = urlencode($goods_name);
								$trade_goods_data = $tools->encode("trade_code=".$TRADE_CODE."&part_idx=".$withlist_row->part_idx."&goods_idx=".$withlist_row->goods_idx."&goods_code=".$withlist_row->goods_code."&goods_name=".$encode_goods_name."&goods_price=".$withlist_row->goods_price."&goods_cnt=".$withlist_row->goods_cnt."&goods_point=".$withlist_row->goods_point."&opt_data=".$withlist_row->opt_data."&box=".$withlist_row->box."&seller=".$goods_stat->seller);
								$db->insert("cs_trade_tmp", "code='$TRADE_CODE', item=0, data='$trade_goods_data', register=now()");
							...
							<ul class="data_cell">
								...
								<li class="point"><span><?=number_format($withlist_row->goods_price*$withlist_row->goods_point*0.01);?></span>point</li>
						...
						<div class="order_user">
							...
							<table>
								...
								<tr>
									<th>전화번호</th>
									<td>
										<input name="order_tel1" class="u_phone" type="text" maxlength="4" size="4" value="<?=$member_stat->tel1;?>" onKeyPress="if( (event.keyCode<48) || (event.keyCode>57) ) event.returnValue=false;">
										<span>-</span>
						...
						<div class="delivery_info" style="display:none;">
							...
							<table>
								...
								<tr class="u_phone">
									<th>전화번호</th>
									<td>
										<input name="deliv_tel1" class="enter" type="text" maxlength="4" size="4" onKeyPress="if( (event.keyCode<48) || (event.keyCode>57) ) event.returnValue=false;"/>
										<span>-</span>
								...
								<tr class="u_phone">
									<th>핸드폰</th>
									<td>
										<input name="deliv_phone1" type="text" class="enter" maxlength="4" size="4" onKeyPress="if( (event.keyCode<48) || (event.keyCode>57) ) event.returnValue=false;"/>
										<span>-</span>
								...
								<tr class="u_post">
									<th>우편번호</th>
									<td>
										<input name="deliv_zip" id="deliv_zip" type="text" class="enter" maxlength="5" size="5" onKeyPress="if( (event.keyCode<48) || (event.keyCode>57) ) event.returnValue=false;">
										<a href="javascript:sample2_execDaumPostcode()" title="우편번호검색">우편번호검색</a>
									</td>
								</tr>
								<tr class="u_address">
									<th>배송지 주소</th>
									<td>
										<input name="deliv_add1" id="deliv_add1" type="text" class="enter">
									</td>
								</tr>
								<tr class="u_address">
									<th>나머지 주소</th>
									<td><input name="deliv_add2" type="text" class="enter" placeholder='나머지 주소'></td>
								</tr>
								<tr class="comment">
									<th>주문시 요청사항</th>
