	...
	if($_GET[cart_method]==1) {
		...
		if(($goods_stat->unlimit==0) && ($goods_stat->number < ($cart_goods_cnt->goods_cnt + $_POST[buy_goods_cnt]))) {
			...
			if(empty($cart_goods_cnt->goods_cnt)) {
				// 따음표 문제 해결
				$db->insert("cs_cart", "userid='$_SESSION[USERID]', code='$_SESSION[CART]', part_idx=$part_idx, goods_idx=$idx, goods_code=$goods_stat->code, goods_name='$goods_stat_name', goods_price=$shopPrice, goods_point=$goods_stat->point, goods_cnt=$_POST[buy_goods_cnt], opt_data='$Optdata', box=$goods_stat->box, seller='$goods_stat->seller', deliv_exc='$goods_stat->deliv_exc', register=now()");
			} else {
...
<div class="site-wrapper" id="box_wrapper">
	...
	<!--컨텐츠 출력1 시작-->
	<section id="blog" class="section cart_area">
					...
					<div class="order_list_area">
						...
						<form name="form_<?=$form_cnt;?>" method="post">
							...
							<ul class="data_cell">
								...
								<li class="prd_photo">
									<a href="product_view.php?goods_data=<?=$goods_data;?>" class="product-image"><img src="../data/goodsImages/<?=$goods_info->images1?>" border="0"></a>
								</li>
								...
								<li class="prd_ea">
									<div>
										...
										<input name="edit_goods_cnt" class="num" type="text" value="<?=$cart_row->goods_cnt;?>" onKeyPress="if( (event.keyCode<48) || (event.keyCode>57) ) event.returnValue=false;">
								...
								<li class="btn">
								<a  href="javascript:cartEdit(document.form_<?=$form_cnt;?>, <?=$cart_row->idx;?>, <?=$cart_row->goods_idx;?>);" onfocus="this.blur()" alt="새로계산" class='new'>새로계산</a>
								<a href="<?=$_SERVER[PHP_SELF];?>?goods_data=<?=$mv_data;?>&cart_method=3&cart_edit_idx=<?=$cart_row->idx;?>" onfocus="this.blur()" alt="삭제하기" class='del'>삭제</i></a>
								</li>
							</ul>
						</form>
						<? }?>
