				...
				<div class="to_animate">
					...
					<div class="main">
						...
						<!--결제방법및 배송정보-->
							...
							<div style="display:none;" class='my_orderview_mobile'  style="text-align:left;">
								...
								<table width="100%" style='border:2px solid #333333;margin:2px;'>
									...
									<tr>
										<td class="order-viewboxIcon jointable_td_border4">
											결제 확인일 : 
											<? if($trade_stat->trade_money_ok !='0000-00-00 00:00:00') {?><?=$tools->strDateCut($trade_stat->trade_money_ok, 5);?><?} else {?><? if($trade_stat->trade_stat==1) {?><span class='Btn_orderlist1'>결제대기</span><?} else if($trade_goods_stat->trade_stat==2){?><span class='Btn_orderlist2'>결제확인</span><?} else if($trade_goods_stat->trade_stat==3 && $trade_goods_stat->invoice_stat == 0){?><span class='Btn_orderlist3'>배송중</span><?} else if($trade_goods_stat->trade_stat==3 && $trade_goods_stat->invoice_stat == 1){?> <span class='Btn_orderlist3'>배송완료</span> <?} else if($trade_goods_stat->trade_stat==4){?><span class='Btn_orderlist4'>판매완료</span><?}?><?}?>
										</td>
									</tr>
									<tr>
										<td class="order-viewboxIcon jointable_td_border4">
											상품 발송일 : <? if($trade_goods_stat->deliv_day !='0000-00-00 00:00:00') {?><?=$tools->strDateCut($trade_goods_stat->deliv_day, 5);?><?} else {?><? if($trade_stat->trade_stat==1) {?><span class='Btn_orderlist1'>결제대기</span><?} else if($trade_goods_stat->trade_stat==2){?><span class='Btn_orderlist2'>결제확인</span><?} else if($trade_goods_stat->trade_stat==3 && $trade_goods_stat->invoice_stat == 0){?><span class='Btn_orderlist3'>배송중</span><?} else if($trade_goods_stat->trade_stat==3 && $trade_goods_stat->invoice_stat == 1){?> <span class='Btn_orderlist3'>배송완료</span> <?} else if($trade_goods_stat->trade_stat==4){?><span class='Btn_orderlist4'>판매완료</span><?}?><?}?>
										</td>
									</tr>
									<tr>
										<td class="order-viewboxIcon jointable_td_border6">
											판매 완료일 : <? if($trade_goods_stat->trade_end_day !='0000-00-00 00:00:00') {?><?=$tools->strDateCut($trade_goods_stat->trade_end_day, 5);?><?} else {?><? if($trade_stat->trade_stat==1) {?><span class='Btn_orderlist1'>결제대기</span><?} else if($trade_goods_stat->trade_stat==2){?><span class='Btn_orderlist2'>결제확인</span><?} else if($trade_goods_stat->trade_stat==3 && $trade_goods_stat->invoice_stat == 0){?><span class='Btn_orderlist3'>배송중</span><?} else if($trade_goods_stat->trade_stat==3 && $trade_goods_stat->invoice_stat == 1){?> <span class='Btn_orderlist3'>배송완료</span><?} else if($trade_goods_stat->trade_stat==4){?><span class='Btn_orderlist4'>판매완료</span><?}?><?}?>
										</td>
									</tr>
								</table>
							</div>
						...
						<!--상품정보-->
							<div>
								<table class='order_view_table order_view_border jointable_all'>
									<tr class="order_view_flex">
										
										
										
												
										<td width="" align="center" class='code jointable_td_border1' bgcolor='ffffff'>
											<p>주문번호</p><?=$trade_stat->trade_code;?>
										</td>
										
										<td width="" align="center" class='code jointable_td_border1' bgcolor='ffffff'>
											<p>주문접수일</p><?=$trade_stat->trade_day;?>
										</td>		
									...
									?>
										<td width="300"  class='subject jointable_td_border1'>
											<p>상품명</p><?=$db->stripSlash($trade_goods_row->goods_name);?>&nbsp;
											 
										</td>
										<td class="jointable_td_border1">
											 <p>수량</p><?=number_format($trade_goods_row->goods_cnt);?> 개
										</td>
										<td  class="jointable_td_border1">
											<p>수령자</p><?=$trade_stat->deliv_name;?>
										</td>
										<td class="jointable_td_border1">
											<p>이메일</p><?=$trade_stat->deliv_email;?>
										</td>
										<td class="code jointable_td_border1">
											<p>연락처</p><? if($trade_stat->deliv_phone1) 	{?><?=$trade_stat->deliv_phone1;?><?=$trade_stat->deliv_phone2;?><?=$trade_stat->deliv_phone3;?><?}?>
										</td>
										<td class="jointable_td_border1">
											<p>주소</p>(<?=$trade_stat->deliv_zip;?>) &nbsp;<?=$trade_stat->deliv_add1;?> <?=$trade_stat->deliv_add2;?>
										</td>
										
										<td class="code jointable_td_border1">
											<p>배송예약일</p><? if($trade_stat->deliv_pree_day!='0000-00-00 00:00:00') {?><?=$tools->strDateCut($trade_stat->deliv_pree_day, 5);?><?}?>
										</td>
										<td class="code jointable_td_border1">
											<p>송장번호</p><font color='#C70000'><?=$trade_stat->trade_number;?></font>&nbsp;&nbsp;&nbsp;&nbsp;<?if($trade_stat->trade_number){?><a href="<?=$content = str_replace("__EXPRESS_NUM__", $trade_stat->trade_number, $admin_stat->delivery_url);?>" target="new" class='Btn_orderlist1'>배송조회</a><?}?>
										</td>
										<td class="jointable_td_border1">
											<p>요청사항</p><?=$trade_stat->deliv_content;?>
										</td>
										...
										<td align="center" class="jointable_td_border1">
											<span>
												(구매금액: <?=number_format($trade_stat->trade_total_price);?> 원) + 
												(배송비: <?=number_format($trade_stat->trade_deliv_price);?> 원) =
												(결제금액: <?=number_format($trade_stat->trade_price);?> 원)
											</span>
										</td>

										<td style="justify-content: center !important;" align="center" class="menupurple jointable_td_border1" bgcolor='ffffff'>
											<? if($trade_stat->trade_stat==1) {?><span style="width: auto;" class='Btn_orderlist1'>결제대기</span>
											<?} else if($trade_goods_stat->trade_stat==2){?><span style="width: auto;" class='Btn_orderlist2'>결제확인</span>
											<?} else if($trade_goods_stat->trade_stat==3 && $trade_goods_stat->invoice_stat==0){?><span style="width: auto;" class='Btn_orderlist3'>배송중</span>
											<?} else if($trade_goods_stat->trade_stat==3 && $trade_goods_stat->invoice_stat==1){?><span style="width: auto;" class='Btn_orderlist3'>배송완료</span>
											<?} else if($trade_goods_stat->trade_stat==4){?><span style="width: auto;" class='Btn_orderlist4'>판매완료</span>
											<?} else if($trade_goods_stat->trade_stat==5){?><span style="width: auto;" class='Btn_orderlist4'>거래취소요청중</span>
											<?} else if($trade_goods_stat->trade_stat==51){?><span style="width: auto;" class='Btn_orderlist4'>환불 대기중</span>
											<?} else if($trade_goods_stat->trade_stat==52){?><span style="width: auto;" class='Btn_orderlist4'>거래취소/환불완료</span>
											<?}?>
											<? if($trade_goods_stat->trade_stat == 3){?>
												<a href="javascript:send_state();" class='Btn_orderlist5'>구매확정</a>
											<?}?>
											<a style="display:none;" href="product_view.php?part_idx=<?=$trade_goods_row->part_idx;?>&goods_data=<?=$goods_data;?>"  class='Btn_orderlist5'>상품평작성</a>
											<span style="display:none;" class="Btn_orderlist5" onclick="view_memo(<?=$frm_num;?>)">내 문의</span>
										</td>
									</tr>
									<style>
										.table_comm td{
									...
									<tr id="View_memo<?=$frm_num;?>" align="center" bgcolor='ffffff' class='calendar_list_tableTD_bgright' style="display:none;">
										<td colspan="7">
											...
											<table width="100%" class='table_comm'>
												<tr align="center"  bgcolor="1379cd" style="border:1px solid #333; height:30px;"> 
													<td style="color:#fff;" width="150" >
														작성일
													</td>
													<td style="color:#fff;">
														내용
											...
											<table width="100%">
												...
												<tr>
													<td style="width:80%; text-align:center;">
														<textarea name="content" style="width:95%;height:50px;padding:10px; resize: none;" placeholder="등록된 글은 수정, 삭제가 되지 않습니다."></textarea>	
																												
													</td>
													<td width="100" height="100" style="padding-bottom:10px; " >
														<a href="javascript:document.write_<?=$frm_num;?>_form.submit();">
														<p class="order_view_qa_btn">
															등록
														</p>
														</a>
													</td>
						...
						<!--주문자정보-->
							...
							<div style="display:none;" class='my_orderview_mobile'  style="text-align:left;">
								...
								<table width="100%" style='border:2px solid #333333;margin:2px;'>
									...
									<tr>
										<td class="order-viewboxIcon jointable_td_border4">
											주문 사항 : <?=$trade_stat->deliv_content;?>
										</td>
									</tr>
									<tr>
										<td class="order-viewboxIcon jointable_td_border4">
									...
									<tr>
										<td class="order-viewboxIcon jointable_td_border6">
											배송장번호 : <font color='#C70000'><?=$trade_stat->trade_number;?></font>&nbsp;&nbsp;&nbsp;&nbsp;<?if($trade_stat->trade_number){?><a href="<?=$content = str_replace("__EXPRESS_NUM__", $trade_stat->trade_number, $admin_stat->delivery_url);?>" target="new" class='Btn_orderlist1'>배송조회</a><?}?>
										</td>
									</tr>
								</table>
							</div>
