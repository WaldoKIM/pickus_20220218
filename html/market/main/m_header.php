    ...
    $(document).ready(function (){
        ...
        $(".mobile_header .top_area h1 img").each(function (){
            $(this).load(function (){
                ...
                $('.mobile_header .top_area h1').css({ 'max-width':imgWidth })
            });
        });
        //스크롤시 검색 숨기기
        ...
        $(window).scroll(function (){
            if ($(document).scrollTop() > st.top){
                ...
                $('.mobile_header .top_area div.right_area a.search_btn').css({ 'visibility':'visible' });
            }
            else{
                $('.mobile_header').removeClass('fixed');
                $('.mobile_header .top_area div.right_area a.search_btn').css({ 'visibility':'hidden' });
            }
        });
		//검생창 open
		$(".right_area a.search_btn").click(function () {
			$('.m_toggle_shopcate').animate({ 'left':'0' },600);
...
    	};
		...
		$('.m_toggle_shopcate .m_toggle_close').click(function () {
			$('.m_toggle_shopcate').animate({ 'left':'-100%' });
...
<div id="Mobile-Header">
    <header class="mobile_header">
		...
		<div class="m_toggle_shopcate">
			...
			<div class="top_info">
				<? if($_SESSION[USERID]){?>
					<a href="login_ok.php?logout=1">[로그아웃]</a>
				<? } else{?>
					<a href="login.php">[로그인]<span>로그인 해주세요</span></a>
				<? }?>
			...
			<ul class="member_menu">
				<? if($_SESSION[USERID]){?>
					<li><a href="my_member_edit.php"><i class="far fa-user icon"></i>회원정보수정<i class="fas fa-angle-right arrow"></i></a></li>
				<? } else{?>
					<li><a href="join.php"><i class="far fa-user icon"></i>회원가입<i class="fas fa-angle-right arrow"></i></a></li>
				<? }?>
				<?
					$cartCnt = "";
					$navresult	= $db->select("cs_navigation", "where openg=1 order by ranking asc" );
			...
			<div class="m_todaylist">
				...
				<ul>
					<?
						...
						$show_arr = explode("&&", $HTTP_COOKIE_VARS[$show_cookie_name]);
						$show_cnt = 3;
						...
						if($HTTP_COOKIE_VARS[$show_cookie_name]){
					...
					<li>
						<a href='product_view.php?part_idx=<?=$goods_info->part_idx;?>&goods_data=<?=$quick_data;?>'>
							<dl>
								<dt><img src='thumbnail.img.php?ThumbEncode=<?=$ThumbEncode?>'/></dt>
								<dd>
									<span class="prd_name"><?=$tools->strCut($goods_info->name,55);?></span><!--제품명-->
									<span class="prd_price"></span><!--가격-->
								</dd>
							</dl>
						</a>
					</li>
