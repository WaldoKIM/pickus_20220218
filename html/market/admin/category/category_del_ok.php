...
if($_GET[idx]) {
	...
	if( $part_stat->part_index == 1 ) {
		...
		while( $del_row = @mysqli_fetch_object($del_result) ) {
			...
			while( $goods_row = @mysqli_fetch_object( $goods_result )) {
				// 상품 리뷰 삭제
				$review_result = $db->select("cs_goods_review", "where goods_idx='$goods_row->idx'");
				while( $review_row = @mysqli_fetch_object( $review_result )) {
					if( $review_row->bbs_file) { @unlink($ROOT_DIR."/data/bbsData/".$review_row->bbs_file); }
					$db->delete("cs_goods_review", "where idx='$review_row->idx'");
				}
				//상품QNA삭제
				$db->delete("cs_goods_qna", "where goods_idx='$goods_row->idx'");
