...
if( $_POST[part1_code] && $_POST[part2_code]  && $_POST[part3_code] ) {	
	...
	$sql="part_name='$_POST[part_name]', goods_cnt='$_POST[goods_cnt]', list_base_sort='$_POST[list_base_sort]', part_display_check='$_POST[part_display_check]', url='$_POST[url]', fieldlist='$fielddata' where idx='$_POST[idx]'";
	if( $db->update("cs_part", $sql) ) { $tools->alertMetaGo("3차 카테고리 수정 되었습니다.", "category_list.php"); } else { $tools->errMsg('비상적으로 입력 되었습니다.');}
} else {
	$tools->errMsg('경 고 !!!\n\n비정상적으로 접근했습니다.');
}
?>
