...
$edit_row = $db->object("cs_part_fixed", "where idx='$_GET[idx]'");
...
function listView() {
	var form=document.part_form;
	if( form.list_display_check[0].checked ) {
		document.all.list_view[0].style.display="none"; 
		document.all.list_view[1].style.display="none"; 
	} else if( form.list_display_check[1].checked ) {
		document.all.list_view[0].style.display=""; 
		document.all.list_view[1].style.display="none"; 
	} else if( form.list_display_check[2].checked ) {
		document.all.list_view[0].style.display=""; 
		document.all.list_view[1].style.display=""; 
	}
}
						...
						<table  width="100%">
							<tr>
								<td>
									<!--콘텐츠출력-->
	<table width="100%">
											<form action="fixed_edit1_ok.php" method="post" name="part_form" enctype="multipart/form-data">
												...
												<table width="100%">
														...
														<td>
															<table width="100%">
																<tr>
																	...
																	<table>
																		<tr>
																			<td class="sub_titleM"><a name="teb01"></a><img src="../img/left_menu_2013icon4.gif" align="absmiddle" border="0">1차카테고리수정</td>
																		</tr>
																	</table>
																	</td>
																</tr>
																					...
																					<td bgcolor="#E9F2F8">
																						<table width="100%">
																							...
																							<tr>
																								<td>
																								<?if($idx==1){?>
																								Category Display : 노출하지 않을 경우에는 주메뉴(상단메뉴)에서는 제외되지만 메인의 기획전 베너 및 기획전 시스템은 그대로 이용됩니다.<br>
																								<?}?>
																								<?if($idx==2){?>
																								Category Display : 노출하지 않을 경우에는 하위 메뉴도 함께 노출되지 않게 되지만, 메인에서의 이벤트상품은 그대로 활성화됩니다. 단지 주메뉴(상단메뉴)에서 노출만 제어합니다.<br>
																								<?}?>
																								</td>
																							</tr>
																						</table>
													...
													<table width="100%" class="table_all">
														<tr bgColor="white"> 
															...
															<td class='tabletd_all tabletd_small'>&nbsp;&nbsp;<font color="#FF0000">1차 카테고리 수정</font></td>
														</tr>
														<tr bgColor="white"> 
															...
															<td class='tabletd_all tabletd_small'>
																&nbsp;한글 : <input name="part_name" type="text" class='formText' maxlength="100" size="30" value="<?=$tools->strHtmlNo($edit_row->part_name);?>">&nbsp;&nbsp;이벤트명을 적어주세요(예:베스트상품)<br>
																 <? if( $edit_row->idx != 1 ) {?>&nbsp;영문 : <input name="part_ename" type="text" class='formText' maxlength="100" size="30" value="<?=$tools->strHtmlNo($edit_row->part_ename);?>">&nbsp;&nbsp;영문이벤트명을 적어주세요(예:Best Products)<br><?}?>
															</td>
														</tr>
														<tr bgColor="white" style="display:none"> 
															...
															<td class='tabletd_all tabletd_small'>&nbsp;<input name="part1_code" type="text" class='formText' maxlength="20" size="30" value="<?=$edit_row->part1_code?>" readonly <? $style->colorAlign('#666666', 0);?>>&nbsp;&nbsp;카테고리 코드는 수정할수 없습니다.</td>
														</tr>
														<tr bgColor="white" style="display:none"> 
															<td width="120" height="35" align="center" bgcolor="E4E7EF" class='contenM tabletd_all'>리스트 출력</td>
															<td class='tabletd_all tabletd_small'>&nbsp;<input type="radio" name="list_display_check" value="0" onclick="javascript:listView();" checked>&nbsp;TEXT 로 출력</td>
														</tr>
														<tr bgColor="white"> 
