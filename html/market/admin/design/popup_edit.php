									...
									<table width="100%">
										<tr>
											<td height="20" class='sub_titleL'>
												<img src="../img/left_menu_2013icon3.gif" align="absmiddle" border="0" hspace="5">팝업창수정
											</td>
										</tr>
										<tr>
...
<table width="100%" border="0" align="center">
	<form action="popup_edit_ok.php" method="post" name="popup_form" enctype="multipart/form-data">
		...
		<td align="center" valign="top" bgcolor="#FFFFFF" class="menu">
				...
				<td>
					<table width="100%">
						<tr>
							...
							<table>
								<tr>
									<td class="sub_titleM"><a name="teb01"></a><img src="../img/left_menu_2013icon4.gif" align="absmiddle" border="0">팝업창수정</td>
								</tr>
							</table>
							</td>
						</tr>
			...
			<table width="100%" class="table_all"> 
				<tr> 
					...
					<td class='tabletd_all tabletd_small'><input type="checkbox" name="popup_display" value="1"  <? if( $row->popup_display == 1 ) { echo("checked");}?>> 모바일 같이 사용시 선택하여 주세요. <br><font color="red">체크가 없을 경우 PC형태에서만 노출됩니다.</font>  </td>
				</tr>
				<tr> 
					<td width="15%" height="25" align="center" bgcolor="E4E7EF" class='contenM tabletd_all'>형태 선택</td>
					<td class='tabletd_all tabletd_small'><input type="radio" name="display" value="0" onClick="popupReg()" <? if( $row->display == 0 ) { echo("checked");}?>> HTML <input type="radio" name="display" value="1" onClick="popupReg()" <? if( $row->display == 1 ) { echo("checked");}?>> 단일 이미지 </td>
				</tr>
				<tr> 
					<td height="25" align="center" bgcolor="E4E7EF" class='contenM tabletd_all'>시작일/종료일</td>
					<td class='tabletd_all tabletd_small'> 
						<select name="start_year"><? for($i=date("Y", $row->start_day);$i<date('Y')+2;$i++){	$today_y=date("Y", $row->start_day);?><option value="<?=$i?>" <?if($i==$today_y) echo("selected");?>><?=$i?></option><?}?></select>&nbsp;년&nbsp;&nbsp;
						<select name="start_mon"><? for($i=1;$i<13;$i++){if(strlen($i)==1)$i="0".$i; $today_m=date("m", $row->start_day);?><option value="<?=$i?>" <?if($i==$today_m) echo("selected");?>><?=$i?></option><?}?></select>&nbsp;월&nbsp;&nbsp;
						<select name="start_day"><? for($i=1;$i<32;$i++){if(strlen($i)==1)$i="0".$i; $today_d=date("d", $row->start_day);?><option value="<?=$i?>" <?if($i==$today_d) echo("selected");?>><?=$i?></option><?}?></select>&nbsp;일&nbsp;~&nbsp;
						<select name="end_year"><? for($i=date("Y", $row->start_day);$i<date('Y')+2;$i++){	$today_y=date("Y", $row->end_day);?><option value="<?=$i?>" <?if($i==$today_y) echo("selected");?>><?=$i?></option><?}?></select>&nbsp;년&nbsp;&nbsp;
						<select name="end_mon"> <? for($i=1;$i<13;$i++){if(strlen($i)==1)$i="0".$i; $today_m=date("m", $row->end_day);?><option value="<?=$i?>" <?if($i==$today_m) echo("selected");?>><?=$i?></option><?}?></select>&nbsp;월&nbsp;&nbsp;
						<select name="end_day"><? for($i=1;$i<32;$i++){if(strlen($i)==1)$i="0".$i; $today_d=date("d", $row->end_day);?><option value="<?=$i?>" <?if($i==$today_d) echo("selected");?>><?=$i?></option><?}?></select>&nbsp;일
					</td>
				</tr>
				<tr> 
					<td height="25" align="center" bgcolor="E4E7EF" class='contenM tabletd_all'>팝업창 사이즈</td>
					<td class='tabletd_all tabletd_small'><input name="width" type="text" class="formText" size="3" <?=$style->colorAlign("#000000", 0);?> <?=$style->strCheck(0);?> value="<?=$row->width;?>"> 가로 X <input name="height" type="text" class="formText" size="3" <?=$style->colorAlign("#000000", 0);?> <?=$style->strCheck(0);?> value="<?=$row->height;?>"> 세로 &nbsp;&nbsp;&nbsp;(새로운 창의 크기를 설정해주세요)<br>
					*입력된 사이즈는 PC에서만 사용되며, 모바일에서는 100% 고정입니다.</td>
				</tr>
				<tr> 
					<td height="25" align="center" bgcolor="E4E7EF" class='contenM tabletd_all'>팝업창 위치</td>
					<td class='tabletd_all tabletd_small'><input name="top" type="text" class="formText" size="3"  value="<?=$row->top;?>" <?=$style->colorAlign("#000000", 0);?> <?=$style->strCheck(0);?>>TOP <input name="lefts" type="text" class="formText" size="3" <?=$style->colorAlign("#000000", 0);?> <?=$style->strCheck(0);?>  value="<?=$row->lefts;?>"> LEFT &nbsp;&nbsp;&nbsp;<br>
				...
				<tr> 
					...
					<td class='tabletd_all tabletd_small'><input name="title_bar" type="text" class="formText  formText_subject" value="<?=$row->title_bar;?>"> &nbsp;간단한 설명</td>
				</tr>
				<tr> 
					<td height="25" align="center" bgcolor="E4E7EF" class='contenM tabletd_all'>쿠키 설정</td>
					<td class='tabletd_all tabletd_small'><input type="radio" name="live" value="0" <? if( $row->live == 0 ) { echo("checked");}?>>※ 오늘은 이창을 다시 띄우지 않음 &nbsp;<input type="radio" name="live" value="1" <? if( $row->live == 1 ) { echo("checked");}?>>※ 이창은 다시는 띄우지 않음</td>
				</tr>
				<tr id="popup_view" style="display:none;"> 
					<td height="25" align="center" bgcolor="E4E7EF" class='contenM tabletd_all'>링크URL</td>
					<td class='tabletd_all tabletd_small'>HTTP://<input name="link_url" type="text" class="formText formText_subject" value="<?=$row->link_url;?>"></td>
				</tr>
				<tr id="popup_view" style="display:none;"> 
					<td height="25" align="center" bgcolor="E4E7EF" class='contenM tabletd_all'>출력 이미지</td>
					<td class='tabletd_all tabletd_small'><? if( $row->popup_images) { ?><a href="../../data/designImages/<?=$row->popup_images;?>" rel="lightbox" class='btn_guide1'>이미지보기</a><? }?>&nbsp;<input name="popup_images" type="file" class="formText" size="30"></td>
				</tr>
				<tr id="popup_view" style="display:none;"> 
			...
			<table width="100%">
				<tr id="popup_view" style="display:none;"> 
					<td height="35" align="center" class='contenM'><img src="../images/arrow.gif" width="3" height="5"> 단일이미지 업로드시 링크URL 을 입력하시면 이미지를 클릭할 경우 해당페이지로 이동시킵니다. </td>
				</tr>
			</table><br>
			<table width="100%">
				<tr> 
