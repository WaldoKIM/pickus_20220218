...
// 파일업로드 
	...
	// 디비에 입력
	if($_POST[pwd]){
		if( $db->update("cs_bbs_data", "subject='$_POST[subject]', homepage='$_POST[homepage]', name='$_POST[name]', hold='$_POST[hold]', category='$_POST[category]', pwd=PASSWORD('$_POST[pwd]'), email='$_POST[email]', tag='$_POST[tag]', notice='$_POST[notice]', content='$_POST[content]', bbs_file='$file_name', add_file1='$add_file1', add_file2='$add_file2', add_file3='$add_file3', add_file4='$add_file4', add_file5='$add_file5' where idx=$idx") ) { $tools->alertJavaGo("수정 하였습니다.", "bbs_list.php?bbs_data=$mv_data"); } else { $tools->errMsg('비상적으로 입력 되었습니다.');}
	}else{
		if( $db->update("cs_bbs_data", "subject='$_POST[subject]', homepage='$_POST[homepage]', name='$_POST[name]', hold='$_POST[hold]', category='$_POST[category]', email='$_POST[email]', tag='$_POST[tag]', notice='$_POST[notice]', content='$_POST[content]', bbs_file='$file_name', add_file1='$add_file1', add_file2='$add_file2', add_file3='$add_file3', add_file4='$add_file4', add_file5='$add_file5' where idx=$idx") ) { $tools->alertJavaGo("수정 하였습니다.", "bbs_list.php?bbs_data=$mv_data"); } else { $tools->errMsg('비상적으로 입력 되었습니다.');}
	}
} else {
	$tools->errMsg('경 고 !!!\n\n비정상적으로 접근했습니다.');
}
