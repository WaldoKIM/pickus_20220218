		...
		<table border="0"  width="100%">
			<tr>
				<td height="20" class='sub_titleL'>
					<img src="../img/left_menu_2013icon3.gif" align="absmiddle" border="0" hspace="5">상품Q&A관리
				</td>
			</tr>
			<tr>
	...
	<tr> 
		...
		<table width="100%">
			...
			<table width="100%" class="table_all">
				...
				if( $search_item == 1 ) {
					$totalList	= $db->cnt( "cs_goods_qna", "where goods_name like '%$search_order%'" );
					$result		= $db->select( "cs_goods_qna", "where goods_name like '%$search_order%' order by idx desc LIMIT $startPage, $listScale" );
				} else if( $search_item == 2 ) {
					$totalList	= $db->cnt( "cs_goods_qna", "where goods_code like '$search_order'" );
					$result		= $db->select( "cs_goods_qna", "where goods_code like '$search_order' order by idx desc LIMIT $startPage, $listScale" );
				} else if( $search_item == 3 ) {
					$totalList	= $db->cnt( "cs_goods_qna", "where title like '%$search_order%'" );
					$result		= $db->select( "cs_goods_qna", "where title like '%$search_order%' order by idx desc LIMIT $startPage, $listScale" );
				} else if( $search_item == 4 ) {
					$totalList	= $db->cnt( "cs_goods_qna", "where userid like '$search_order'" );
					$result		= $db->select( "cs_goods_qna", "where userid like '$search_order' order by idx desc LIMIT $startPage, $listScale" );
				} else { 
					$totalList	= $db->cnt( "cs_goods_qna", "" );
					$result		= $db->select( "cs_goods_qna", "order by idx desc LIMIT $startPage, $listScale" );
				}
				...
				<tr bgColor="white" align="center" height="25"> 
					...
					<td width="95" class='tabletd_all tabletd_Lmall'><a href="qna_edit.php?review_data=<?=$review_data;?>" class="menusmall_btn3">수정</a>&nbsp;<a href="javascript:reviewDel('<?=$review_data;?>');" class="menusmall_btn4">삭제</a></td>
				</tr>
				</form>
				<?
					$listNo--;	
