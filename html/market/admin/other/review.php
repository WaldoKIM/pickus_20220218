...
// 상품리뷰레벨 수정(level) 변수
if($_POST[hidden_level_idx]) { $db->update("cs_goods_review", "main='$_POST[main]' where idx='$_POST[hidden_level_idx]'");}
...
function reviewDel( mv_data ) {
	...
	if(choose) {	location.href='review_del_ok.php?review_data='+mv_data; }
	else { return; }
}
////  회원에게 메일 보내기 ///////////////////////////////////////////////////////////////////////////////
		...
		<table border="0"  width="100%">
			<tr>
				<td height="20" class='sub_titleL'>
					<img src="../img/left_menu_2013icon3.gif" align="absmiddle" border="0" hspace="5">제품사용후기
				</td>
			</tr>
			<tr>
		...
		<table width="100%">
				...
				<td>
					<table width="100%">
						<tr>
							...
							<table>
								<tr>
									<td class="sub_titleM"><a name="teb01"></a><img src="../img/left_menu_2013icon4.gif" align="absmiddle" border="0">제품사용후기</td>
								</tr>
							</table>
							</td>
						</tr>
			...
			<table width="100%" class="table_all">
				<tr height='40'> 
					...
					<td bgcolor="E4E7EF" class='contenM tabletd_all'>메인설정</td>
					...
					<td bgcolor="E4E7EF" width="10%" class='contenM tabletd_all revbox_none'>평점</td>
					<td bgcolor="E4E7EF" class='contenM tabletd_all revbox_none'>ID</td>
					<td bgcolor="E4E7EF" width="10%" class='contenM tabletd_all'>작성일</td>
					<td width="95" bgcolor="E4E7EF" class='contenM tabletd_all'>관리</td>
				</tr>
				<?
				$listScale			=	15; 		// 리스트갯수
				$pageScale		=	15;		// 페이지 갯수
				...
				if( $search_item == 1 ) {
					$totalList	= $db->cnt( "cs_goods_review", "where goods_name like '%$search_order%'" );
					$result		= $db->select( "cs_goods_review", "where goods_name like '%$search_order%' order by idx desc LIMIT $startPage, $listScale" );
				} else if( $search_item == 2 ) {
					$totalList	= $db->cnt( "cs_goods_review", "where goods_code like '$search_order'" );
					$result		= $db->select( "cs_goods_review", "where goods_code like '$search_order' order by idx desc LIMIT $startPage, $listScale" );
				} else if( $search_item == 3 ) {
					$totalList	= $db->cnt( "cs_goods_review", "where title like '%$search_order%'" );
					$result		= $db->select( "cs_goods_review", "where title like '%$search_order%' order by idx desc LIMIT $startPage, $listScale" );
				} else if( $search_item == 4 ) {
					$totalList	= $db->cnt( "cs_goods_review", "where userid like '$search_order'" );
					$result		= $db->select( "cs_goods_review", "where userid like '$search_order' order by idx desc LIMIT $startPage, $listScale" );
				} else { 
					$totalList	= $db->cnt( "cs_goods_review", "" );
					$result		= $db->select( "cs_goods_review", "order by idx desc LIMIT $startPage, $listScale" );
				}
				...
				<tr bgColor="white" align="center" height="25"> 
					...
					<td class='tabletd_all tabletd_Lmall'>
						<select name="main" class="input" onChange="javascript:authChange(document.form_<?=$form_name?>);">
							<option value="1" <? if( $row->main == 1 ) { echo("selected");} ?>>메인출력</option>
							<option value="0" <? if( $row->main == 0 ) { echo("selected");} ?>>미출력</option>
						</select>
					</td>
					<td class='tabletd_all tabletd_Lmall revbox_none'>
					<a href="http://<?=$admin_stat->shop_url?>/product_view.php?goods_data=<?=$goods_data;?>" target="_NEW"><font color='909090'><?=$goods_stat->name;?></font></a></td>
					<td class='tabletd_all tabletd_Lmall'  style='text-align:left;'>
					...
					<?}?>
					<?=$row->title;?><a href="javascript:reviewView('<?=$review_data;?>');" class="menusmall_btn3" style='margin:3px;'>답글달기</a><br><span class='request_noneoolim'><a href="http://<?=$admin_stat->shop_url?>/product_view.php?goods_data=<?=$goods_data;?>" target="_NEW"><font color='909090'><?=$goods_stat->name;?></font></a></span></td>
					<td class='tabletd_all tabletd_Lmall revbox_none' align="left"><?for($i=1;$i<=$row->star;$i++){?><img src='../images/star.gif' border='0'><?}?></td>
					
					<td class='tabletd_all tabletd_Lmall revbox_none'><font color='7AA9E9'><?=$row->userid;?></font></td>					
					
					<td class='tabletd_all tabletd_Lmall'>
					<span class='revbox_on' style='text-align:center;'><?for($i=1;$i<=$row->star;$i++){?><img src='../images/star.gif' border='0'><?}?><br>
					<font color='7AA9E9'><?=$row->userid;?></font></span>
					
					<font color='E35858'><?=$tools->strDateCut($row->register,1);?></font></td>
					
					<td width="95" class='tabletd_all tabletd_Lmall'><a href="review_edit.php?review_data=<?=$review_data;?>" class="menusmall_btn3">수정</a>&nbsp;<a href="javascript:reviewDel('<?=$review_data;?>');" class="menusmall_btn4">삭제</a></td>
				</tr>
				</form>
				<?
					$listNo--;	
				...
				<tr bgColor="white" align="center"> 
					<td height="100" colspan="7" align="center"> 등록한 질문이 없습니다.</td>
				</tr>
				<?}?>
			</table>
