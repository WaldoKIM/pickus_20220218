...
if($_POST[refund_bank]) {
	$db->update("cs_trade+goods", "refund_bank='$_POST[refund_bank]' where idx=$trade_idx");
	$tools->msg('환불계좌 입력완료');
	?>
	<script>
...
function inputBank() {
	var form=document.refund_form;
	form.refund_bank.value = "<?=$member_stat->bank;?>/<?=$member_stat->account_num;?>/<?=$member_stat->account_name;?>";
}
//-->
</script>
<body>
			...
			<table width="95%"  class="table_all">
				...
				<tr> 
					...
					<td class='contenM tabletd_all'>
						 <a href="javascript:inputBank()" class="menusmall_btn1">회원정보와 동일</a>
						<input name="refund_bank" type="text" class="formText_mo" size="40" maxlength="50" value="<?=$refund_bank;?>">
					</td>
				</tr>
				<tr> 
					<td class='contenM tabletd_all' height="35" colspan="2" align="center"><span  style='text-align: center;'><a href="javascript:sendit();" class="itemtable_default_bt3">등록</a></span></td>
				</tr>
				</form>
			</table>
		</td>
