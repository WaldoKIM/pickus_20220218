		...
		<td height="150" align="center" valign="top" bgcolor="#FFFFFF" class="menu"><br>
			...
			<table width="630" class="menu" bordercolor='#BDBEBD' style='border-collapse: collapse'>
				<tr> 
					...
					<td align="center" class="menupurple"><? if($trade_stat->trade_stat==1) {?>결제대기중<?} else if($trade_stat->trade_stat==2){?>결제확인됨<?} else if($trade_stat->trade_stat==3){?>배송중<?} else if($trade_stat->trade_stat==4){?>판매완료됨<?} else if($trade_stat->trade_stat==5){?>서비스교환중<?} else if($trade_stat->trade_stat==6){?>반품취소됨<?}?></td>
				</tr>
			</table><br>
			<table width="630">
				<tr> 
			...
			<table width="630" bordercolor='#BDBEBD' style='border-collapse: collapse'>
				<tr align="center" bgcolor="F2F2F2"> 
					<td height="25">서비스코드</td>
					<td height="25">서비스명</td>
					<td height="25">옵 션</td>
					<td height="25">가 격</td>
					<td height="25">수 량</td>
			...
			<table width="630" bordercolor='#BDBEBD' style='border-collapse: collapse'>
				<tr> 
					<td width="110" height="25" align="center" bgcolor="F2F2F2">적립포인트</td>
					<td width="140" align="center"><? if($trade_stat->order_userid) {?><? if($trade_stat->trade_stat==4) {?> <?=number_format($trade_stat->trade_save_point);?> ⓟ 적립<?} else {?><?=number_format($trade_stat->trade_save_point);?> ⓟ 예정<?}?><?} else {?><font color="#FF9933">비회원</font><?}?></td>
					<td width="100" align="center" bgcolor="F2F2F2">구매금액</td>
					<td width="140" align="center"><?=number_format($trade_stat->trade_total_price);?> 원</td>
					<td width="110" align="center" bgcolor="F2F2F2">사용포인트</td>
					<td width="100" align="center"><? if($trade_stat->order_userid) {?><?=number_format($trade_stat->trade_use_point);?> ⓟ<?} else {?><font color="#FF9933">비회원</font><?}?></td>
					<td width="100" align="center" bgcolor="F2F2F2">배송비</td>
					<td width="100" align="center"><?=number_format($trade_stat->trade_deliv_price);?> 원</td>
				</tr>
				<tr> 
					<td height="35" colspan="3" align="center" bgcolor="F2F2F2"><? if($trade_stat->order_userid) {?>회원할인율  : <?=number_format($trade_stat->trade_member_dc);?> 원<?}?></td>
					<td height="35" colspan="5" align="center" bgcolor="F2F2F2">결제금액  : <?=number_format($trade_stat->trade_price);?> 원</td>
				</tr>
			</table><br>
			<table width="630">
				<tr> 
			...
			<table width="630" bordercolor='#BDBEBD' style='border-collapse: collapse'>
				<tr> 
					<td width="100" height="25" align="center" bgcolor="F2F2F2">결제방법</td>
					<td width="215" height="25"><? if($trade_stat->trade_method==1){;?>카드결제<?} else if($trade_stat->trade_method==2){;?>계좌이체<?} else if($trade_stat->trade_method==3){;?>휴대폰결제<?} else if($trade_stat->trade_method==4){;?>가상계좌 : <?=$trade_stat->trade_method_info;?><?} else if($trade_stat->trade_method==5){;?>무통장입금 : <?=$trade_stat->trade_method_info;?><?} else if($trade_stat->trade_method==6){;?>포인트결제<?}?></td>
					<td width="100" align="center" bgcolor="F2F2F2">결제 확인일</td>
					<td width="215"><? if($trade_stat->trade_money_ok !='0000-00-00 00:00:00') {?><?=$tools->strDateCut($trade_stat->trade_money_ok, 5);?><?} else {?><? if($trade_stat->trade_stat==1) {?>결제대기중<?} else if($trade_stat->trade_stat==2){?>결제확인됨<?} else if($trade_stat->trade_stat==3){?>배송중<?} else if($trade_stat->trade_stat==4){?>판매완료됨<?} else if($trade_stat->trade_stat==5){?>서비스교환중<?} else if($trade_stat->trade_stat==6){?>반품취소됨<?}?><?}?></td>
				</tr>
				<tr> 
					<td height="25" align="center" bgcolor="F2F2F2">배송일</td>
					<td width="215" height="25"><? if($trade_stat->trade_start_day !='0000-00-00 00:00:00') {?><?=$tools->strDateCut($trade_stat->trade_start_day, 5);?><?} else {?><? if($trade_stat->trade_stat==1) {?>결제대기중<?} else if($trade_stat->trade_stat==2){?>결제확인됨<?} else if($trade_stat->trade_stat==3){?>배송중<?} else if($trade_stat->trade_stat==4){?>판매완료됨<?}?><?}?></td>
					<td align="center" bgcolor="F2F2F2">판매 완료일</td>
					<td width="215"><? if($trade_stat->trade_end_day !='0000-00-00 00:00:00') {?><?=$tools->strDateCut($trade_stat->trade_end_day, 5);?><?} else {?><? if($trade_stat->trade_stat==1) {?>결제대기중<?} else if($trade_stat->trade_stat==2){?>결제확인됨<?} else if($trade_stat->trade_stat==3){?>배송중<?} else if($trade_stat->trade_stat==4){?>판매완료됨<?} else if($trade_stat->trade_stat==5){?>서비스교환중<?} else if($trade_stat->trade_stat==6){?>반품취소됨<?}?><?}?></td>
				</tr>
			</table><br>
