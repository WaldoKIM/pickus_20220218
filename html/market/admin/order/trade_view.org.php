											...
											<td height="150" align="center" valign="top" bgcolor="#FFFFFF" class="menu">
													<table border="0" width="100%">
																...
																<div class="customertable_divcenter_1">
																	<table width="100%" class="table_all table_all_moA">
																		<tr bgColor="white"> 
																			...
																			<td width="20%"  bgcolor="E4E7EF" class='contenM tabletd_all'>거래상태</td>
																			<td class='tabletd_all tabletd_Lmall'><? if($trade_stat->trade_stat==1) {?>결제대기중<?} else if($trade_stat->trade_stat==2){?>결제확인됨<?} else if($trade_stat->trade_stat==3){?>작업물 배송중<?} else if($trade_stat->trade_stat==4){?>판매완료됨<?}?></td>
																		</tr>
																	</table>
																</div>
															</div>
														...
														<table width="100%" class="table_all">
															<tr align="center"  bgcolor="E4E7EF" class='contenM tabletd_all'> 
																...
																<?/*
																<td height="35" class='contenM tabletd_all noneoolimmoL'>작업물 전송</td>
																*/?>
															</tr>
															<?
															...
															<tr align="center" bgColor="white"> 
																...
																<span class='noneoolimmoL_on' style='text-align:center'><span class='font1'>가격 : <?=number_format($trade_goods_row->goods_price);?> 원</span><br>
																<span class='font4'>수량 : <?=number_format($trade_goods_row->goods_cnt);?>개</span><br>
																<span class='font3'>구매금액 : <?=number_format($trade_goods_row->goods_price*$trade_goods_row->goods_cnt);?> 원</span>
																</td>
																...
																<?/*
																<td height="35" class='tabletd_all tabletd_Lmall noneoolimmoL' style="text-align:left;">
																	<form name="trade_file_<?=$frm_num;?>_form" action="trade_view_file_ok.php?trade_data=<?=$mv_data;?>" method="post" enctype="multipart/form-data">
																		<input type="hidden" name="idx" value="<?=$trade_goods_row->idx;?>">
																	<?if($trade_goods_row->upfile != "" && $trade_goods_row->upfile != "none"){
																		$trade_file = explode( "&&", $trade_goods_row->upfile );
																		?>
																		<!--
																		<p style="padding:5px;">																			
																		첨부파일 : <a href="trade_view_download.php?idx=<?=$trade_goods_row->idx;?>"><?=$trade_file[1];?></a> 
																		<input type="checkbox" name="del_upfile" value="1"> 삭제(삭제우선!)
																		</p>
																		-->
																	<?}?>
																		<p style="padding:5px;">
																			<input type="file" name="upfile" class="formfile"> 
																			<a href="javascript:document.trade_file_<?=$frm_num;?>_form.submit();" class="btn_guide1">확인</a>
																		</p>
																	</form>
																</td>
																*/?>
														...
														<table border="0" width="100%">
															<tr>
																...
																<table>
																	<tr>
																		<td class="sub_titleM"><a name="teb01"></a><img src="../img/left_menu_2013icon4.gif" align="absmiddle" border="0">배송정보</td>
																	</tr>
																</table>
																</td>
															</tr>
												...
												<table width="100%" class="table_all">
													...
													<tr>
														...
														<td class='tabletd_all tabletd_Lmall'><? if($trade_stat->trade_money_ok !='0000-00-00 00:00:00') {?><?=$tools->strDateCut($trade_stat->trade_money_ok, 5);?><?} else {?><? if($trade_stat->trade_stat==1) {?>결제대기중<?} else if($trade_stat->trade_stat==2){?>결제확인됨<?} else if($trade_stat->trade_stat==3){?>작업물 배송중<?} else if($trade_stat->trade_stat==4){?>판매완료됨<?}?><?}?></td>
													</tr>
													<tr bgColor="white"> 
														<td height="35" align="center"  bgcolor="E4E7EF" class='contenM tabletd_all'>작업물 발송일</td>
														<td height="35" class='tabletd_all tabletd_Lmall'><? if($trade_stat->trade_start_day !='0000-00-00 00:00:00') {?><?=$tools->strDateCut($trade_stat->trade_start_day, 5);?><?} else {?><? if($trade_stat->trade_stat==1) {?>결제대기중<?} else if($trade_stat->trade_stat==2){?>결제확인됨<?} else if($trade_stat->trade_stat==3){?>작업물 배송중<?} else if($trade_stat->trade_stat==4){?>판매완료됨<?}?><?}?></td>
													</tr>
													<tr>
														<td align="center"  bgcolor="E4E7EF" class='contenM tabletd_all'>판매 완료일</td>
														<td  class='tabletd_all tabletd_Lmall'><? if($trade_stat->trade_end_day !='0000-00-00 00:00:00') {?><?=$tools->strDateCut($trade_stat->trade_end_day, 5);?><?} else {?><? if($trade_stat->trade_stat==1) {?>결제대기중<?} else if($trade_stat->trade_stat==2){?>결제확인됨<?} else if($trade_stat->trade_stat==3){?>작업물 배송중<?} else if($trade_stat->trade_stat==4){?>판매완료됨<?}?><?}?></td>
													</tr>
												</table>
															</form>
