...
?>
<link rel="stylesheet" type="text/css" href="/estimate/css/lg.css?after">
<div>
	<div class="banner_search_div">
		<div class="banner"><img src="/estimate/img/LG.png" alt=""></div>
		<div class="search">
			<input class="search_product" type="text" id="model_code" placeholder="모델명을 입력해주세요.">
	...
	<form action="estimate_registerLGT2.php" method="post">
				...
				success: function(data) {
					...
					for (var objs of obj) {
						...
						Object.keys(objs).forEach(function(v) {
							...
							if (v == '등록년월') year.push(objs[v]);
						})
					}
					if(average_price != null){
					var yearstr = year[0];
					var year = yearstr.substring(0,5);
					$('#search_name').text(name[0]);
					$('#search_code').text(code[0]);
					$('#search_brand').text(brand[0]);
					//$('#search_category1').text(category1[0]);
					$('#search_category2').text(category2[0] + " " + category3[0]);
					$('#search_year').text(year);
					$('#search_price').text(average_price + "원");
					$('#search_price1').text(average_price + "원");

					$('#search_namer').val(name[0]);
					$('#search_coder').val(code[0]);
					$('#search_brandr').val(brand[0]);
					$('#search_category2r').val(category2[0] + " " + category3[0]);
					$('#search_yearr').val(year);
