...
//    $sql = " select * from {$g5['estimate_list_photo']} where estimate_idx = '$idx' order by idx limit 1 "; //대표이미지를 첫번째로. 마지막 이미지 오류가능성
    $row = sql_fetch($sql);
...
function estimate_img_thumbnail($et_img, $width, $height)
{
    ...
    return '<img src="'.G5_DATA_URL.'/estimate/'.$file_new_path.'/'.$thumb_img.'" style="width:100%;">';
