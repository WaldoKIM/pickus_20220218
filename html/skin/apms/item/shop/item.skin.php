...
<?php if($nav_title) { ?>
	<aside class="item-nav">
		<span class="page-nav pull-left text-muted">
...
<div class="item-head">
	<div class="row">
		...
		<div class="col-sm-6" style="padding-left: 0;">
			<div class="view_name">
			<div class="state if_no">새상품</div>
			<div class="title"><?php echo stripslashes($it['it_name']); // 상품명 ?></div>
				<?php if (!$it['it_use']) { // 판매가능이 아닐 경우 ?>
					<div class="price">
			            <span class="after">판매중지</span>
			        </div>
					
				<?php } else if ($it['it_tel_inq']) { // 전화문의일 경우 ?>
					<div class="price">
			            <span class="after">전화문의</span>
			        </div>
				<?php } else { // 전화문의가 아닐 경우?>
					<?php if ($it['it_cust_price']) { ?>
						<div class="price">
				            <span class="after"><?php echo display_price($it['it_cust_price']); ?></span>
				        </div>
					<?php } // 시중가격 끝 ?>
						<div class="price">
				            <span class="after"><?php echo display_price(get_price($it)); ?></span>
			...
			<?php if ($it['it_use_avg']) { ?>
				<!-- <dl>
					<dt>고객평점</dt>
					<dd><?php echo apms_get_star($it['it_use_avg'],'fa-lg red'); //평균별점 ?></dd>
				</dl> -->
				<dl>
					<dt>물품 보증 기간(A/S)</dt>
					<dd><?php echo "3개월"; ?></dd>
			...
			<?php if($it['it_buy_min_qty']) { ?>
				<dl><dt>최소구매수량</dt><dd><?php echo number_format($it['it_buy_min_qty']); ?> 개</dd></dl>
			<?php } ?>
			<?php if($it['it_buy_max_qty']) { ?>
				<dl><dt>최대구매수량</dt><dd><?php echo number_format($it['it_buy_max_qty']); ?> 개</dd></dl>
			<?php } ?>
			<?php
			...
			?>
			<dl>
				<dt><?php echo $ct_send_cost_label; ?></dt><dd><?php echo $sc_method; ?></dd>
			</dl>
			<div id="item_option">
				<?php if($option_item) { ?>
				...
				<?php if ($is_orderable) { ?>
					<div id="it_sel_option">
						...
						if(!$option_item) {
							if(!$it['it_buy_min_qty'])
								$it['it_buy_min_qty'] = 1;
						?>
							<ul id="it_opt_added" class="list-group view_option">
								<li class="it_opt_list list-group-item">
									...
									<div class="row">
										<div class="col-sm-7">
											<label style="margin-top: 5px;" id="qt_pro">
												상품 수량
											</label>
										</div>
										<div class="col-sm-5 counter">
											<div class="input-group counter_box">
												<label for="ct_qty_<?php echo $i; ?>" class="sound_only" style="display: none;">수량</label>
												<input type="text" name="ct_qty[<?php echo $it_id; ?>][]" value="<?php echo $it['it_buy_min_qty']; ?>" id="ct_qty_<?php echo $i; ?>" class="form-control updown_input" size="5">
												<span class="updown"><a href="#none"  class="btn_up it_qty_plus" title="더하기"></a><a href="#none" class="btn_down it_qty_minus" title="빼기"></a></span>
													<!-- <button type="button" ><i class="fa fa-plus-circle fa-lg"></i><span class="sound_only">증가</span></button> -->
													<!-- <button type="button" class=""><i class="fa fa-minus-circle fa-lg"></i><span class="sound_only">감소</span></button> -->
											</div>
										</div>
									</div>
									<?php if($it['pt_msg1']) { ?>
					...
					<!-- 총 구매액 -->
					<dl>
						<dt>총 금액</dt><dd><span id="it_tot_price">0원</span></dd>
					</dl>
				<?php } ?>
			</div>
			...
			<?php if ($is_orderable) { ?>
				<div style="overflow:hidden; text-align:center; padding:12px 0;">
					...
					<li><a href="#" class="btn btn-<?php echo $btn1;?> btn-block" onclick="apms_wishlist('<?php echo $it['it_id']; ?>'); return false;"><img src="/img/wish.png"></a></li>
				...
				<div id="area_partner" style="width: 100%; height: 60px; background-color: #ccc">
					<h3 style="text-align: center;">파트너 영역</h3>
				</div>
...
<div id="item-tab" class="div-tab tabs<?php echo ($wset['tabline']) ? '' : ' trans-top';?>">
	<ul class="nav nav-tabs nav-justified">
		...
		<?php if($is_comment) { // 댓글 ?>
			<li style="display: none;"><a href="#item-cmt" data-toggle="tab"><b>댓글<?php echo $it_comment_cnt;?></b></a></li>
		<?php } ?>
		<?php if($is_ii) { // 상품정보고시 ?>
			<li style="display: none;"><a href="#item-info" data-toggle="tab" ><b>필수표기정보</b></a></li>
		<?php } ?>
	...
	<div class="tab-content" style="border:0px; padding:20px 0px;">
		...
		<?php if($is_comment) { // 댓글 ?>
			<div class="tab-pane" id="item-cmt" style="display: none;">
				<div id="icv"></div>
				<?php include_once('./itemcomment.php'); ?>
			</div>
		<?php } ?>
		<?php if($is_ii) { // 상품정보고시 ?>
			<div class="tab-pane" id="item-info" style="display: none;">
