...
<style type="text/css">
	#mobileInfo1 .ma,#mobileInfo1 .pe{display: inline-block;}
	.ma, .pe{margin-right: 10px;}
	dd .pe{margin-left: 15px;}
	#board .view .info dd,#board .view .info dt{height: 44px;}
	#content{padding: 10px;}
	#modal_price_detail .col-xs-7{margin-top: 13px;}
	.propose_price + .propose_price{margin-top: 5px;}
	#frmPrice p{margin-bottom: 10px !important;}
	@media(max-width: 768px){
		.modal_table .title{font-size: 12px}
	}
</style>
			...
			<div class="view">
				<div class="mob">
					...
					<?php
						...
						if($master['state'] == "0" || $master['state'] == "1" || $master['state'] == "2"){
							if($master['e_type'] =="0"){
								...
								if($master['price_qty'] > 0){
									if(!$propose_process_fetch['price_minus']){
										echo "<p class='text-center'><span class='ma'>매입</span>".display_estimate_price($propose_process_fetch['price'])."</p>";

									}
									if($propose_process_fetch['price_minus']){
										echo "<p class='text-center'><span  class='pe'>폐기</span>".number_format($propose_process_fetch['price_minus'])."원</p>";
									}
								}else{
							...
							if($master['e_type'] =="2"){
								...
								if($master['price_qty'] > 0){
									if($propose_process_fetch['price']){
										echo "<p class='text-center'>".display_estimate_price($propose_process_fetch['price'])."</p>";
									}
								}else{
									echo "<p class='text-center'>-원</p>";
								}
							...
							if($master['e_type'] =="0" && !$propose_success['price_minus']){
										echo "<p class='text-center'><span class='ma'>매입</span>".display_estimate_price($price)."</p>";

									}else if($master['e_type'] =="0" && $propose_success['price_minus']){
										echo "<p class='text-center'><span class='pe'>폐기</span>".number_format($price_minus)."원</p>";
										...
										if($price_minus){
											echo "<span class='pe'>폐기</span>".number_format($price_minus)."원";
										}
										echo "</p>";
									}else{
					...
					<div class="customer">
						...
						<?php
							echo "<dt class='col-xs-1 main_co'>마감일</dt>";
				...
				<table class="web">
					<tr>
						...
						<td class="info" id="mainTitle">
							...
							<?php
								...
								if($state == "0" || $state == "1" || $state == "6"){
										...
										if($e_type == "2"){
											...
											if( $centerCnt >  0 ){
												if($e_type == "0" || $e_type == "1"){
													...
													if($propose_process_fetch['price_minus']){
														echo "<span  class='pe'>폐기</span>".number_format($propose_process_fetch['price_minus'])."원";	
													}
													echo "</dd>";
												}else{
									...
									if($e_type == "2"){
										...
										if( $centerCnt >  0 ){
											if($e_type == "0" && $propose_process_fetch['price_minus'] != 0 && $master['price_qty'] == 1){
												echo "<dt class='col-xs-3'>최고견적</dt><dd class='col-xs-9'>".number_format($propose_process_fetch['price_minus'])."</dd>";	
											}else{
									...
									if($master['e_type'] =="0" && !$propose_success['price_minus']){
										echo "<dd class='col-xs-9'><span class='ma'>매입</span>".display_estimate_price($price)."</dd>";

									}else if($master['e_type'] =="0" && $propose_success['price_minus']){
										echo "<dd class='col-xs-9'><span class='pe'>폐기</span>".number_format($price_minus)."원</dd>";
									}else if($master['e_type'] =="1"){
									...
									echo "<dt class='col-xs-3'>참여자</dt><dd class='col-xs-9'>".$master['price_qty']."명</dd>";
									echo "<dt class='col-xs-3'>선택견적가</dt><dd class='col-xs-9'>".display_estimate_price($price)."</dd>";
									if($master['e_type'] != "2"){
										echo "<dt class='col-xs-3'>선택폐기가</dt><dd class='col-xs-9'>".display_estimate_price($price_minus)."</dd>";
									}
								}
							...
							<?php }else{ //간편견적 ?>
								<dt class='col-xs-3'>이름</dt><dd class='col-xs-9'><?php echo $master['nickname'] ?></dd>
								<dt class='col-xs-3'>이메일</dt><dd class='col-xs-9'><?php echo $master['email'] ?></dd>
								<dt class='col-xs-3'>연락처</dt><dd class='col-xs-9'><?php echo $master['phone'] ?></dd>
								<dt class='col-xs-3'>설명</dt><dd class='col-xs-9'><?php echo $master['content'] ?></dd>
								...
								if(!$propose_success['price'] && !$propose_success['price_minus']){
									...
									if($e_type =="2"){
										...
										if($propose_success['price']){
											echo "<div class='pay'><span class='white ma'>매입</span>".number_format($propose_success['price'])."원</div>";

										}
										if($propose_success['price_minus']){
											echo "<div class='pay'><span class='white pe'>폐기</span>".number_format($propose_success['price_minus'])."원</div>";
										}
									}
								}
								...
								if($row['meet']){
									...
									if(!$row['price'] && !$row['price_minus']){
										...
										if($e_type =="2"){
											...
											if($row['price']){
												echo "<div class='pay'><span class='white ma'>매입</span>".number_format($row['price'])."원</div>";
											}	
											if($row['price_minus']){
												echo "<div class='pay'><span class='white pe'>폐기</span>".number_format($row['price_minus'])."원</div>";
											}
										}
									}
								}
				...
				?>
				<?php
						$sql = " select a.*, case when b.rc_email is not null then 'Y' else 'N' end as request_yn, b.estimate_yn from {$g5['member_table']} a left join {$g5['estimate_request']} b on a.mb_email = b.rc_email and b.estimate_idx = '$idx' and mb_show_type = '1'";
						if($e_type == "0" || $e_type == "1"){
						...
						for ($i=0; $row=sql_fetch_array($request_list); $i++) {
							...
							echo "<li>";
							echo "<div>";
							echo "<div class='img'><img src='/data/estimate/". $row['mb_photo_site'] . "'></div>";
							echo "<div class='text'>";
							if($score > 0 && $score_row['cnt'] > 0)
							{
								if($score < 1){
			    ...
			    <div><?php echo $member['mb_bank']; ?> - <?php echo $member['mb_bank_num']; ?></div>
		...
		<div class="modal-content">
			<div class="modal-body">
				...
				<form name="frmselect" action="<?php echo G5_URL; ?>/estimate/my_estimate_form_select.php" method="post" enctype="multipart/form-data" autocomplete="off">
					...
					<input type="hidden" id="biz_name" name="biz_name">
					...
					<div style="display: none;" class="text-center" id="area_after">
						<p style=" margin: 15px 0 30px;">업체 진행 완료 후 고객님의 계좌로 입금됩니다.</p>
						<div class="form-group">
		...
		<div class="modal-content">
			<div class="modal-body">
				...
				<form name="frmselect_normal" action="<?php echo G5_URL; ?>/estimate/my_estimate_form_select.php" method="post" enctype="multipart/form-data" autocomplete="off">
					<input type="hidden" id="idx"     name="idx" value="<?php echo $master['idx']; ?>">
					<input type="hidden" id="sub_idx" name="sub_idx">
					<input type="hidden" id="biz_name_normal" name="biz_name_normal">
					<div class="text-center" id="area_before">
						<i class="xi-error-o"></i>
				...
				<div class="btn_wrap">
					<ul class="row">
						<li class="col-xs-3 col-xs-offset-3"><a class="line_bg" href="#." data-dismiss="modal">닫기</a></li>
						<li class="col-xs-3" id="confirm_after"><a class="main_bg"  href="javascript:doSelectComplete();">확인</a></li>
					</ul>
				</div>
