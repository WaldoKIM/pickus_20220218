...
if($no_estimate){
    $w = "u";
    $sql = "select * from {$g5['estimate_match']} a where no_estimate = '$no_estimate' ";
    $estimate = sql_fetch($sql);
    $sql = "select * from g5_estimate_match_info a where no_estimate = '$no_estimate' ";
    $estimate_info = sql_query($sql);
    $readonly = "readonly";
}
...
?>
<form name="fmember" id="fmember" action="./pickus_estimate_add_match_update.php" onsubmit="return fmember_submit(this);" method="post" enctype="multipart/form-data">
        ...
        <table>
            ...
            <tbody>
                <tr>
                    <th>이름</th>
                    <td>
                        <input type="text" name="nickname" value="<?php echo $estimate['name'] ?>" id="nickname" class="frm_input" style="width:100%">
                    </td>
                    <th>이메일</th>
                    <td>
                        <input type="text" name="email" value="<?php echo $estimate['email'] ?>" id="email" class="frm_input" style="width:100%">
                    </td>
                </tr>
                <tr>
                    <th>전화번호</th>
                    <td>
                        <input type="text" name="number" value="<?php echo $estimate['number'] ?>" id="number" class="frm_input" style="width:100%">
                    </td>
                </tr>
                <tr>
                    <th>견적마감일</th>
                    <td>
                        <input type="text" name="deadline" value="<?php echo $estimate['date_close'] ?>" id="deadline" class="frm_input" style="width:100%" placeholder="2020-00-00">
                    </td>
                    <th>배송요청일</th>
                    <td>
                        <input type="text" name="date_req" value="<?php echo $estimate['date_req'] ?>" id="date_req" class="frm_input" style="width:100%" placeholder="2020-00-00">
                    </td>
                </tr>
            </tbody>
        </table>
        ...
        <table>
            ...
            <tbody>
                <tr>
                    ...
                    <th>상세주소</th>
                    <td>
                        <input type="text" name="area3" value="<?php echo $estimate['place'] ?>" id="area3" class="frm_input" style="width:100%">
                    </td>
    ...
    <div class="tbl_frm01 tbl_wrap">
        <table>
            ...
            <tbody>
                <tr>
                    <th>제목</th>
                    <td><input type="text" name="title" id="title" placeholder="제목을 입력해주세요" class="frm_input" style="width:100%"></td>
                </tr>
                <tr>
                    <th>카테고리</th>
                    <td colspan="3">
                ...
                <tr>
                    ...
                    <div class="goods_qy"><input type="text" id="search_goods_" placeholder="입력해주세요">
                        <input type="number" min="1" id="qty"><strong> 개</strong></div>
                ...
                <tr>
                    <th>참고사항</th>
                    <td colspan="3">
                        <textarea id="content" name="content" class="frm_input" style="width:100%;height:400px;"><?php echo $estimate['etc_req'] ?></textarea>
                    </td>
                </tr>
            </tbody>
        </table>
...
<style type="text/css">
    .add_pro{margin-bottom: 3%; border-bottom: 1px solid #ececec;}
    .add_pro div{float: left;}
    .add_pro .add_name{width: 60%; text-align: center;}
    .add_pro .add_qty{width: 20%;}
    .add_pro .remove_pro{width: 20%;}
</style>
...
$(function() {
    ...
    $("#add_goods").click(function(){
        ...
        vHtml += "<input type='hidden' name='cate_name[]' value='"+$("#search_goods_").val()+"'>";
        vHtml += "<input type='hidden' name='qty[]' id='qty_hidden' value='"+$("#qty").val()+"'>";
        vHtml += "<div class='add_name'><strong>"+$("#search_goods_").val()+"</strong></div>";
